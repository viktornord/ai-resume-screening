...
- **Symmetric extract** — JD and each CV get the **same shape** of structured object + **per-section confidence**.
- **Match from structure + raw** — build the match prompt from `Requirements` + `CandidateProfile`; always attach **full JD and full CV text**; where confidence is low, instruct the model to **trust raw text** for that section.
- **KISS orchestration** — no judge LLM, no deterministic scorer, no DB; three prompt types only.
...
flowchart TD
  ...
  CV["LLM per resume: CV to CandidateProfile"]
  Match["LLM per resume: match"]
...
| Each CV → `CandidateProfile` | **N** (parallel, max 3) | Same schema philosophy as JD |
| Each CV → match result | **N** (parallel, max 3) | Inputs below; can run extract+match back-to-back per file |
| Rank | 0 | Threshold 70 |

**Call count:** `1 + 2N` per batch (F saves re-parsing JD; symmetric extract is the cost).

**Per-file sequencing:** For each resume, run `CandidateProfile` extract then `match` before moving to the next slot in the semaphore (keeps prompts small and debuggable).
...
Every LLM call uses **strict JSON** whose fields must be documented for the model (same text in `jd_extract.txt`, `resume_extract.txt`, `screening.txt`, and as Pydantic `Field(description=...)` / JSON Schema `description` for Ollama `format: json`).
...
Copy this block (or a shortened version) into `resume_extract.txt`, `jd_extract.txt`, and `screening.txt` under **EXTRACTION RULES** / **MATCHING RULES**.
...
Built from [prompts/jd_extract.txt](backend/app/prompts/jd_extract.txt). Used to **compose** the match prompt (not a dump of raw JD alone).
...
### `CandidateProfile` (from each CV — one LLM call per file)

Same sections, candidate-side semantics. Prompt: [prompts/resume_extract.txt](backend/app/prompts/resume_extract.txt).
...
### Match step (per resume — one LLM call)

**Inputs to [prompts/screening.txt](backend/app/prompts/screening.txt):**

1. `Requirements` JSON  
2. `CandidateProfile` JSON  
3. Full **raw JD** text  
4. Full **raw CV** text (truncate ~8k chars if needed)  
5. **Low-confidence section names** (from section `confidence` only)  
6. **`Requirements.ambiguities`** and **`CandidateProfile.ambiguities`** (object-level only)  

**Output** (maps to public API — includes reasoning for recruiters):
...
1. Serialize `Requirements` into a short **REQUIREMENTS** block (must/nice tech with `min_years`, `experience.min_total_years`, leadership, education).  
2. Serialize `CandidateProfile` into a **CANDIDATE** block.  
3. Append **RAW JOB DESCRIPTION** and **RAW RESUME**.  
4. Append **LOW CONFIDENCE SECTIONS** (section names only).  
5. Append **JD AMBIGUITIES** and **CV AMBIGUITIES** from each object’s root `ambiguities[]`.

Extract/match prompts: include **FIELD REFERENCE** + **Prompt constraints** sections. If uncertain, lower section `confidence`, document in object `ambiguities[]` and `reasoning`, do not silently guess.
...
| `resume_extract.py` | CV text → `CandidateProfile` |
| `llm_screening.py` | Match: requirements + profile + raw + low-confidence notes → candidate result |
...
2. **Backend** — parser + 3 prompts + F/2N pipeline + mock LLM + tests  
...
- Fixture CV → valid `CandidateProfile` JSON  
